...
class TaskController extends AbstractController
{
    ...
    #[Route('/tasks/search', name: 'search_tasks_by_category')]
    public function searchTasksByCategory(Request $request, TaskRepository $taskRepository): Response
    {
        $categoryName = $request->query->get('categoryName');
        
        if (!$categoryName) {
            throw $this->createNotFoundException('Category name not provided');
        }
        
        $tasks = $taskRepository->findByCategoryName($categoryName);
        
        return $this->render('task/search_tasks_by_category.html.twig', [
