...
class TaskController extends AbstractController
{
    ...
    #[Route("/task/by-category/{categoryid}", name: "taskByCategory")]
    public function searchByCategory($categoryid, CategoryRepository $categoryRepo)
    {
        $category = $categoryRepo->findBy($categoryid);
    
        if (!$category) {
            throw $this->createNotFoundException('Category not found');
        }
    
        $tasks = $categoryRepo->getTasks(); // Assuming you have a relationship between Category and Task entities

    return $this->render('task/index.html.twig', [
        'category' => $category,
        'tasks' => $tasks,
        'caegoryid' => $categoryRepo->getId(),
    ]);
